...
func convertAudio(inputData []byte, format string, speed float64) ([]byte, int, error) {
	var cmd *exec.Cmd

	// Prepare speed filter if speed is different from 1.0
	var audioFilter []string
	if speed != 1.0 {
		audioFilter = []string{"-af", fmt.Sprintf("atempo=%.2f", speed)}
	}
	...
	default:
		...
		args = append(args, "-write_xing", "0", "-compression_level", "10", "-application", "voip", "-fflags", "+bitexact", "-flags", "+bitexact", "-id3v2_version", "0", "-map_metadata", "-1", "-map_chapters", "-1", "-write_bext", "0", "pipe:1")
		cmd = exec.Command("ffmpeg", args...)
	}
...
func processAudio(c *gin.Context) {
	...
	// Parse speed parameter with validation
	speedStr := c.DefaultPostForm("speed", "1.0")
	speed, err := strconv.ParseFloat(speedStr, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid speed parameter"})
		return
	}
	
	// Validate speed range (0.25x to 4.0x)
	if speed < 0.25 || speed > 4.0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Speed must be between 0.25 and 4.0"})
		return
	}

	convertedData, duration, err := convertAudio(inputData, format, speed)
	if err != nil {
...
func transcribeOnly(c *gin.Context) {
	...
	convertedData, _, err := convertAudio(inputData, "ogg", 1.0)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
